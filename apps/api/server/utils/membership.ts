 ...
 */
export async function getMembershipInfo(userId: string, organizationId?: string): Promise<MembershipInfo> {
  ...
  try {
    ...
    const activeSubscriptions = customerState.subscriptions?.filter(sub => 
      sub.status === 'active' && 
    ...
    const benefits = customerState.benefits?.map(benefit => benefit.type) || [];
 ...
 */
export async function getUserMembershipStatus(userId: string, organizationId?: string) {
