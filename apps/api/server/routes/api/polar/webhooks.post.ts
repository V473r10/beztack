...
import { handleWebhookRequest, createDefaultWebhookHandlers } from "@nvn/payments/server";
  ...
  try {
    ...
    const webhookHandlers = createDefaultWebhookHandlers({
      onOrderPaid: async (payload) => {
          ...
          try {
            await db.update(schema.organization)
              .set({
                ...
                subscriptionValidUntil: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days default
                updatedAt: new Date()
              })
              .where(eq(schema.organization.id, organizationId));
              
            console.log(`Organization ${organizationId} subscription activated with tier: ${tierName}`);
          } catch (error) {
        ...
        if (payload.data.metadata?.userId) {
          ...
          try {
            ...
            console.log(`User ${userId} subscription activated with tier: ${tierName}`);
          } catch (error) {
      ...
      onSubscriptionActive: async (payload) => {
          ...
          try {
            await db.update(schema.organization)
              .set({
                ...
                subscriptionId: payload.data.id,
                polarCustomerId: payload.data.customer_id,
                subscriptionValidUntil: payload.data.current_period_end ? new Date(payload.data.current_period_end) : null,
                updatedAt: new Date()
              })
              .where(eq(schema.organization.id, organizationId));
              
            console.log(`Organization ${organizationId} subscription activated: ${tierName}`);
          } catch (error) {
            console.error(`Failed to activate organization subscription ${organizationId}:`, error);
          }
        }
        ...
        if (payload.data.metadata?.userId) {
          ...
          try {
            ...
            console.error(`Failed to activate user subscription ${userId}:`, error);
          }
        }
      },
      ...
      onSubscriptionCanceled: async (payload) => {
          ...
          try {
            await db.update(schema.organization)
              .set({
                ...
                subscriptionValidUntil: payload.data.current_period_end ? new Date(payload.data.current_period_end) : new Date(),
                updatedAt: new Date()
              })
              .where(eq(schema.organization.id, organizationId));
              
            console.log(`Organization ${organizationId} subscription canceled, active until: ${payload.data.current_period_end}`);
          } catch (error) {
        ...
        if (payload.data.metadata?.userId) {
          ...
          try {
            ...
            console.log(`User ${userId} subscription canceled, active until: ${payload.data.current_period_end}`);
          } catch (error) {
            console.error(`Failed to cancel user subscription ${userId}:`, error);
          }
        }
      },
      ...
      onCustomerStateChanged: async (payload) => {
        ...
        const userId = payload.data.metadata?.userId || payload.data.metadata?.external_id;
        const organizationId = payload.data.metadata?.referenceId;
        ...
        if (userId) {
          try {
            ...
            const activeSubscription = payload.data.subscriptions?.find((sub: any) => sub.status === 'active');
            if (activeSubscription) {
              updates.subscriptionTier = activeSubscription.metadata?.tier || 'pro';
              updates.subscriptionStatus = 'active';
              updates.subscriptionId = activeSubscription.id;
              updates.subscriptionValidUntil = activeSubscription.current_period_end ? new Date(activeSubscription.current_period_end) : null;
            }
            ...
            console.error(`Failed to update user ${userId} customer state:`, error);
          }
        }
        ...
        if (organizationId) {
          try {
            ...
            const activeSubscription = payload.data.subscriptions?.find((sub: any) => sub.status === 'active');
            if (activeSubscription) {
              updates.subscriptionTier = activeSubscription.metadata?.tier || 'team';
              updates.subscriptionStatus = 'active';
              updates.subscriptionId = activeSubscription.id;
              updates.subscriptionValidUntil = activeSubscription.current_period_end ? new Date(activeSubscription.current_period_end) : null;
            }
            ...
            console.log(`Updated organization ${organizationId} with customer state`);
          } catch (error) {
            console.error(`Failed to update organization ${organizationId} customer state:`, error);
          }
        }
      },
