...
function buildTierConfigFromProducts(products: Product[]): MembershipTierConfig[] {
  const tiers: Record<MembershipTier, MembershipTierConfig> = {
    free: {
      id: "free",
      name: "Free",
  ...
  for (const product of products) {
    ...
    if (metadata?.permissions && Array.isArray(metadata.permissions)) {
      ...
      current.permissions = Object.entries(metadata.permissions)
        .filter(
...
export function MembershipProvider({ children }: MembershipProviderProps) {
  ...
  const tierConfigs = useMemo(
    () => buildTierConfigFromProducts(products),
    [products]
  )
