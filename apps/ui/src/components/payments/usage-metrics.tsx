...
export function UsageMetrics({ 
  ...
  const hasNearLimitUsage = usageData.some(item => 
    item.limit !== -1 && (item.current / item.limit) * 100 >= 80
  );

  const hasOverLimitUsage = usageData.some(item => 
    ...
    <Card className={cn("w-full", className)}>
      <CardHeader className="space-y-1">
        <div className="flex items-center justify-between">
          ...
          {tierConfig.id !== "enterprise" && (hasNearLimitUsage || hasOverLimitUsage) && (
            <Badge 
      ...
      <CardContent className="space-y-6">
        ...
        {tierConfig.id !== "enterprise" && (hasNearLimitUsage || hasOverLimitUsage) && onUpgrade && (
          <div className={cn(
