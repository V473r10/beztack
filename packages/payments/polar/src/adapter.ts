	...
	return {
		...
		async listProducts(): Promise<Product[]> {
			...
			return response.result.items.map((product) => {
				...
				return {
					...
					price: {
						amount:
							(price?.priceAmount ?? 0) / CENTS_TO_DOLLARS,
						currency: price?.priceCurrency ?? "USD",
					},
					interval: mapPolarInterval(
		...
		async createSubscription(
			...
			if (!options.productId) {
				throw new Error(
					"productId is required for Polar subscriptions",
				)
			}
		...
		async updateSubscription(
			...
			return {
				...
				currentPeriodEnd: updated.currentPeriodEnd
					...
					: undefined,
				cancelAtPeriodEnd:
					updated.cancelAtPeriodEnd ?? undefined,
				metadata: updated.metadata as
					| Record<string, unknown>
					| undefined,
			}
		},
		...
		async cancelSubscription(
			...
			return {
				...
				status: immediately
					? "canceled"
					: mapPolarStatus(canceled.status),
		...
		parseWebhook(
			...
			let mappedType: WebhookPayload["type"] =
...
export function createAdapter(
	...
	return createPolarAdapter({
		...
		server: (config.POLAR_SERVER ?? "sandbox") as
			| "sandbox"
			| "production",
		organizationId: config.POLAR_ORGANIZATION_ID ?? "",
		successUrl:
			config.PAYMENTS_SUCCESS_URL ??
			config.POLAR_SUCCESS_URL ??
			"",
		cancelUrl:
			config.PAYMENTS_CANCEL_URL ?? config.POLAR_CANCEL_URL,
	})
}
